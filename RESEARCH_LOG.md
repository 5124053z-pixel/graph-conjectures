...
**12. The same lesson, made structural rather than relearned by hand.**
Failure 11 was caught by eye, on a single three-vertex case. Rather than trust
that every future small counterexample would be noticed the same way, the
check was turned into code: `SUSPICIOUS_ORDER = 6` in `wowii.py` flags any
"counterexample" on a conjecture from a published, database-tested list whose
smallest violation is on 6 vertices or fewer, tagging it `SUSPECT THE CODE`
instead of reporting it as a finding. It fired correctly on its first real use
— see the 489 violations on conjecture 160 immediately below, smallest on 5
vertices, which turned out *not* to be a bug this time, but earned the same
scrutiny a bug would have gotten.
...
"The literature check, done properly — and what it found" below. "holds 671"
and "holds 12111" are correct readings of this 8-vertex scan and nothing more
— they are the two clearest illustrations in this file of why "settled over
all graphs on at most 8 vertices" carries very little information about truth.
...
wowii_status.py       the status of the 22 per DeLaVina's primary source
                       (resolved.htm / open.html), not the Lean `open` tag —
                       this is where the 200 and 291 refutations are checked
wowii_hunt.py         counterexample hunt above the exhaustive range, for
                       every conjecture still open
wowii_ham.py          the four Hamiltonian-path conjectures against five
                       classical certificates (Chvatal-Erdos, Ore, Ls<=2,
                       Bondy-Chvatal closure, Chvatal's degree condition)
wowii_ham_hunt.py     hill-climbing hunt for 194, 198a, 217 above n = 8
status_table.py       generates the coverage table from the toolkit's own
                       bound list, so a claim can't drift from what the code
                       computes (the fix for failure 19's mislabelled line)
wowii19.py            conjecture 19 reduced to one inequality,
                       b(G) >= alpha(G) + alpha(G - I)
wowii2.py             conjecture 2 proved for triangle-free graphs (Theorem C)
                       and shown tight on C_n, K_{a,a}, Q3
wowii59.py            conjecture 59 reduced to the AM-GM strengthening
                       2 f(G) >= b(G) + res(G)
wowii61.py            conjecture 61 settled for diameter <= 3, via f >= alpha + 1
wowii100.py           conjecture 100 proved for alpha >= 16
wowii141.py           conjecture 141 proved: neighbourhood star for girth <= 5,
                       locally-tree-like ball for girth >= 6
wowii142.py           conjecture 142 proved for girth <= 3
wowii144.py           conjecture 144 proved for girth <= 3, plus the girth-4
                       case split
wowii145.py           conjecture 145 proved except for one explicit shape
wowii160.py           the five-vertex counterexample to the Lean transcription
                       of conjecture 160, and why DeLaVina's own reading of
                       c_C4 (definition 73, C4-free indicator) rescues it,
                       leaving a residual of 11
wowii314.py           conjecture 314 resolved: the |S| <= 3 cap and the
                       dominating-edge case
...
residue_domination.py the connected-domination lemma conjecture 217 needs —
                       gamma_c stays bounded when residue = 2, confining the
                       open content of 217 to n <= 10
domination.py         exact gamma and gamma_t by branch and bound, validated
ham_search.py         targeted search for a Hamiltonian-path counterexample;
                       this is the run that surfaced failure 13 (an orphaned
                       process burning cores unnoticed)
scan_products.py      the product conjecture over all pairs of small graphs
scan_products_par.py  parallel finish of that scan over the 7x7 pairs
